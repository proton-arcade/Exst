...
const requestedFrom = params.get("from") || "index.html";
const from = /^(?:index|folder)\.html(?:\?[^#]*)?$/.test(requestedFrom)
  ? requestedFrom
  : "index.html";
...
async function bootGame() {
  ...
  if (!game)
    throw new Error(`No game with ID "${gameId}" exists in data/games.txt.`);
  ...
  const available =
    game.available !== false && (await pathLooksAvailable(game.path));
  if (available) {
    frame.src = game.path;
...
bootGame().catch((error) => {
  document.querySelector(".game-shell").innerHTML =
    `<section class="frame-fallback"><h1>Game could not load</h1><p>${ExstArcade.escapeHtml(error.message)}</p><a class="play-link" href="index.html">Back to arcade</a></section>`;
});
