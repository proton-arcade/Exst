(function () {
  ...
  function parseBlockText(text, defaultType = 'game') {
    ...
    text.split(/\r?\n/).forEach((raw) => {
      ...
      if (['tags', 'games', 'aliases'].includes(key)) {
        current[key] = String(value).split(',').map((part) => part.trim()).filter(Boolean);
      } else {
        current[key] = value;
      }
    });
  ...
  async function loadGames() {
    const textGames = parseBlockText(await fetchText('data/games.txt'), 'game').filter((item) => item.type === 'game');
    return textGames.map(normalizeGame).filter((game) => game.id);
  }

  async function loadFolders() {
    const indexText = await fetchText('data/folders/index.txt');
    const files = indexText.split(/\r?\n/).map((line) => stripComments(line).trim()).filter(Boolean);
    const folders = await Promise.all(files.map(async (file) => {
      const text = await fetchText(`data/folders/${file}`);
      const block = parseBlockText(text, 'folder').find((item) => item.type === 'folder') || {};
  ...
  function getOpenMode() {
    return localStorage.getItem('exst-open-mode') || 'page';
  }
  ...
  function bindOpenModeSelect() {
    ...
    select.addEventListener('change', () => localStorage.setItem('exst-open-mode', select.value));
  }
  ...
  function createGameCard(game, options = {}) {
    ...
    const tags = tagList(game).map((tag) => `<span>${tag}</span>`).join('');
    article.innerHTML = `
      <a class="game-thumb" href="${href}"${target} aria-label="Play ${escapeHtml(game.title)}">
        <img src="${game.icon}" alt="${escapeHtml(game.title)} icon" loading="lazy" onerror="this.src='assets/images/default-game.svg'" />
      </a>
      <div class="game-card-body">
        ...
        <div class="card-actions">
          <a class="play-link" href="${href}"${target}>Play</a>
          <a class="small-link" href="${game.path}" target="_blank" rel="noopener">direct</a>
        </div>
      </div>`;
    return article;
  }
  ...
  window.ExstArcade = { loadArcadeData, parseBlockText, bindOpenModeSelect, createGameCard, gameUrl, getOpenMode, escapeHtml };
}());
